...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\fuchi\\Documents\\PsychoPy\\DigitSpan_touch\\Digit_Span_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    continue_button_2 = visual.ImageStim(
        ...
        ori=0.0, pos=(0, -0.4), size=(0.276,0.074),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    for thisBlock in blocks:
        ...
        for thisTrial in trials:
            ...
            # set up handler to look after randomisation of conditions etc
            response = data.TrialHandler(nReps=1000.0, method='random', 
                extraInfo=expInfo, originPath=-1,
                trialList=[None],
            ...
            for thisResponse in response:
                    ...
                    if recall_touch.status == STARTED:  # only update if started and not finished!
                        ...
                        if buttons != prevButtonState:  # button state changed?
                            ...
                            if sum(buttons) > 0:  # state changed to a new click
                                # check if the mouse was inside our 'clickable' objects
                                gotValidClick = False
                                clickableList = environmenttools.getFromNames([digit0,digit1,digit2,digit3,digit4,digit5,digit6,digit7,digit8,digit9,continue_button_2], namespace=locals())
                                for obj in clickableList:
                                    # is this object clicked on?
                ...
                if recall_touch.isPressedIn(digit0):
                    entered_text += '0'
                elif recall_touch.isPressedIn(digit1):
                    entered_text += '1'
                elif recall_touch.isPressedIn(digit2):
                    entered_text += '2'
                elif recall_touch.isPressedIn(digit3):
                    entered_text += '3'
                elif recall_touch.isPressedIn(digit4):
                    entered_text += '4'
                elif recall_touch.isPressedIn(digit5):
                    entered_text += '5'
                elif recall_touch.isPressedIn(digit6):
                    entered_text += '6'
                elif recall_touch.isPressedIn(digit7):
                    entered_text += '7'
                elif recall_touch.isPressedIn(digit8):
                    entered_text += '8'
                elif recall_touch.isPressedIn(digit9):
                    entered_text += '9'
                elif recall_touch.isPressedIn(continue_button_2):
                    continueRoutine=False
